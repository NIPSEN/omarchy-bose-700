#!/bin/bash
# tests/test_deploy.sh — bose-700-deploy must never write or delete through a
# symbolic link, and must delete only what it is told to.
#
# Usage: tests/test_deploy.sh <path-to-bose-700-deploy>
set -uo pipefail

DEPLOY="${1:?usage: test_deploy.sh <bose-700-deploy>}"
WORK="$(mktemp -d)"
...
ln -s "$VICTIM/keep.txt" "$HOME_DIR/.local/bin/bose-700-ctl"
deploy remove .local/bin bose-700-ctl >/dev/null
check "removes a symlinked file entry, not its target" \
  '[[ ! -L $HOME_DIR/.local/bin/bose-700-ctl && -f $VICTIM/keep.txt ]]'

mkdir -p "$HOME_DIR/.local/bin/bose-700-daemon"
touch "$HOME_DIR/.local/bin/bose-700-daemon/inner"
deploy remove .local/bin bose-700-daemon >/dev/null
check "never removes a directory entry" '[[ -f $HOME_DIR/.local/bin/bose-700-daemon/inner ]]'
